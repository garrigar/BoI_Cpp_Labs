		...
		if (!is_correct_filename(filename))
			...
			if (!fin.is_open())
				...
				while (!fin.eof()) {
					...
					if (!is_correct_word(str)) {				// if incorrect, ignore and print notification
						allright = false;
					} 
					else
					{
						...
						if (words_SI[curLen].find(str) != words_SI[curLen].end())
							allright = false;
						else {
				...
				if (!allright) cout << "Some strings seemed like not a correct words, or there were repeating, so they've been ignored" << endl;
				fin.close();
				// -------------------

				if (words_IS.size() == 1)
					cout << "No words loaded - maybe file is empty or contains no correct words in all" << endl;
				else 
				{
					// ---- DEFINE GRAFS ----
					...
					do {
						...
						if (!is_correct_word(begin))
							cout << "Seems like not a correct word!" << endl;
						else {
...
/*
ofstream fout("neighbours_4.txt");
...
/*
ofstream fout("nounlist_sorted.txt");

for (int i = 1; i <= words_IS.size() - 1; ++i) {
	fout << endl << i << endl;
	for (int j = 0; j < words_IS[i].size(); ++j)
		fout << words_IS[i][j] << ' ';
}
